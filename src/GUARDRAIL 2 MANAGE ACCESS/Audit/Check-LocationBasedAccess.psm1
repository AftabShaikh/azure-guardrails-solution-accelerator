function Get-LocationBasedAccess {
    param (      
    ...
    # Set compliance status and comments based on location CAP result
    if ($PsObjectLocation.ComplianceStatus -eq $true) {
        $IsCompliant = $true
        $Comments = $msgTable.isCompliant + " " + $msgTable.compliantLocationCAP
    }
    else {
        $IsCompliant = $false
        $Comments = $msgTable.isNotCompliant + " " + $msgTable.nonCompliantLocationCAP
    }
